...
// Timer Task that prints the values of the two timers 
//
void timer_evt_task(void *arg)
{
    while(1) {
        ...
        // Read evt values from the isr when they are ready to be read.
        // Use 
...
void tg0_timer0_init()
{
    ...
    /*Set ISR handler*/
    timer_isr_register(timer_group, timer_idx, timer_group0_isr, (void*) timer_idx, ESP_INTR_FLAG_LEVEL1, NULL);
    /*Start timer counter*/
...
void app_main()
{
    // Create a queue to hold the timer values
    timer_queue = xQueueCreate(10, sizeof(timer_event_t));
    // Initialize timer 0
    //tg0_timer0_init();
    // Initialize Timer 1
    tg0_timer1_init();
    // Create the timer and put it into the event list 
    // (tasks that are added onto the task list must run indefintely)
